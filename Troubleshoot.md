...
**Problem**: Speech API throws NotAllowedError constant
...
**Solutions**:
1. This is normal behavior - the API has built-in silence detection (~3 seconds)
2. Speak continuously without pausing
3. Try different browser or device
4. Check browser console for errors logged
5. **Stale State Bug Fixed**: This version uses `speechErrorRef` to prevent race conditions
...
4. Verify code block is valid text (not corrupted)
...
**Problem**: Code block with shell commands doesn't create terminal modal
...
2. Example format:
   ...
   $ npm run dev
   ```
3. The code block must use triple backticks with bash/shell language
