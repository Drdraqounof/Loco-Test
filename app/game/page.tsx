...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      if (ballX - ballSize < 0) {
        aiScore++;
        if (aiScore > 4) { // Only trigger when AI reaches exactly 5
          gameActive = false;
          setGameState({ playerScore, aiScore, gameActive: false, gameOver: true, winner: "AI" });
          gameActive = false; // Prevent further updates
        } else {
      ...
      if (ballX + ballSize > canvas_width) {
        playerScore++;
        if (playerScore > 4) { // Only trigger when player reaches exactly 5
          gameActive = false;
