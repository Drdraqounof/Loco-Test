...
# Existing migrations are applied automatically when the app container starts
# To re-run startup after changing infrastructure:
docker compose up -d --build
...
The app container runs `prisma generate` and `prisma migrate deploy` during startup before `next start`.
